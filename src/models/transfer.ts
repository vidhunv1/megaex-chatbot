...
export default class Transfer extends Model<Transfer> {
  ...
  static async newPayment(
    ...
    let paySign = JWT.sign(
      { userId: userId, currencyCode: currencyCode, amount: amount },
      jwtSecret
    );

    try {
      let payment: Transfer = await this.sequelize.transaction(async function(
        t
      ) {
  ...
  static async getBySecret(secret: string): Promise<Transfer | null> {
    ...
    let payment: Transfer | null = await Transfer.findOne({
  ...
  public static async deletePaymentIfExpired(
    ...
    let logger = new Logger().getLogger();
    if (paymentId) {
      let p: Transfer | null = await Transfer.findOne({
        where: { id: paymentId }
      });
  ...
  public static async deleteExpiredPayments(): Promise<boolean> {
    let p: Transfer[] | null = await Transfer.findAll({
      where: {
        status: "pending",
        createdAt: {
          $lte: moment()
