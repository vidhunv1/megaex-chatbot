...
export default class TMHandler {
  ...
  async handleCallbackQuery(
    ...
    let query: ICallbackQuery = callback.data
  ...
  private async handleConversations(
    ...
    if (!isConversationHandled && msg.text && !msg.text.startsWith("/start")) {
      ...
      [currentContext] = await this.redisClient.hmgetAsync(
  ...
  async onboardUser(msg: TelegramBot.Message, user: User, tUser: TelegramUser) {
    ...
    let ccKeyboardButton: TelegramBot.ReplyKeyboardMarkup = {
      keyboard: [],
      one_time_keyboard: false,
      resize_keyboard: true
    };
    ...
    if (msg.text && isBotCommand(msg)) {
      // entry point
      let langKeyboardButton: TelegramBot.ReplyKeyboardMarkup = {
          keyboard: [],
          one_time_keyboard: false,
          resize_keyboard: true
        },
