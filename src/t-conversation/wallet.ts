...
let walletConversation = async function(
...
let walletCallback = async function(
  msg: TelegramBot.Message,
  user: User,
  tUser: TelegramUser,
  ...
  switch (query.callbackFunction) {
    case ICallbackFunction.CoinSend:
      if (query.coinSend && query.coinSend.coin) {
        ...
        let coinWallet: Wallet | null = await Wallet.findOne({
    ...
    case ICallbackFunction.NewAddress:
      ...
      let cCoin,
        newAddress = null;
      [cCoin] = await redisClient.hmgetAsync(
        cacheKeys.tContext.key,
        cacheKeys.tContext["Wallet.coin"]
      );
      if (cCoin) {
    ...
    case ICallbackFunction.CoinAddress:
      await tBot.sendChatAction(msg.chat.id, "typing");
      let curCoin;
      [curCoin] = await redisClient.hmgetAsync(
        cacheKeys.tContext.key,
        cacheKeys.tContext["Wallet.coin"]
      );
      await tBot.sendMessage(
...
let walletContext = async function(
  msg: TelegramBot.Message,
  user: User,
  tUser: TelegramUser,
  context: string
...
async function sendCurrentAddress(
  ...
  let currentCoin;
  let cacheKeys = new CacheStore(tUser.id).getKeys();
  [currentCoin] = await redisClient.hmgetAsync(
    cacheKeys.tContext.key,
    cacheKeys.tContext["Wallet.coin"]
  );
...
async function handleCoinSend(
  ...
  if (msg && msg.text) {
    if (isInputContext) {
      let amountToPay: number | null = await Market.parseCurrencyValue(
        msg.text,
        coin
      );
      if (!amountToPay) {
      ...
      if (msg.text === user.__("create")) {
        try {
          let { paymentCode } = await Transfer.newPayment(
            user.id,
            coin,
            amount
          );
...
async function handlePaymentClaim(
  ...
  try {
    ...
    let t: TelegramUser | null = await TelegramUser.findOne({
...
async function handleWallet(
  ...
  if (wallet) {
    ...
    if (wallet.blockedBalance > 0) {
      ...
      let blockedBalanceLocal = (wallet.blockedBalance * rate).toLocaleString(
        tUser.languageCode
      );
      message =
        message +
