...
export default class NotificationManager {
  ...
  async sendNotification(notificationType: string, data:any) {
    let tBot = (new TelegramBotApi()).getBot(), user;
    switch (notificationType) {
      case NotificationManager.NOTIF.NEW_TRANSACTION:
        ...
        user = await User.findById(transaction.userId, {include: [TelegramUser]});
        if(!user)
          return;
      ...
      case NotificationManager.NOTIF.PAYMENT_EXPIRED:
        ...
        await tBot.sendMessage(user.telegramUser.id, user.__('notify_expired_payment %s %s', data.amount, data.currencyCode), {parse_mode: 'Markdown'})  
        break;
